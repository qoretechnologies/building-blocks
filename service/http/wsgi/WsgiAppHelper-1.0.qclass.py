...
class WsgiAppHelper:
    # borrowed from gunicorn:util.py
    @staticmethod
    def importApp(module, is_debug = 0):
        ...
        try:
            ...
            raise Exception('AppImportError',
        ...
        if isinstance(expression, ast.Name):
            ...
            if not isinstance(expression.func, ast.Name):
                raise Exception('AppImportError', "Function reference must be a simple name: %r" % obj)
        ...
        try:
            ...
            if is_debug:
                UserApi.logInfo("%s", traceback.format_exc())
            raise Exception('AppImportError', "Failed to find attribute %r in %r." % (name, module))
        ...
        if app is None:
            raise Exception('AppImportError', "Failed to find application object: %r" % obj)

        if not callable(app):
            raise Exception('AppImportError', "Application object must be callable.")
        return app
