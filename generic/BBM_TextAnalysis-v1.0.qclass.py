...
import numpy as np
import random
from qore.Util import substitute_env_vars

class BBM_TextAnalysis:
    def __init__(self):
        oov_tok = '<OOV>'

        self.intents = json.loads(UserApi.getTextFileFromLocation(UserApi.getConfigItemValue("text-analysis-intents")))
        ...
        self.model = tf.keras.models.load_model(substitute_env_vars(UserApi.getConfigItemValue("text-analysis-model")))

    def processInputHash(self, input):
        input['result'] = self.processInput(input[UserApi.getConfigItemValue("text-analysis-hash-event-key")])
        return input
    ...
    def processInput(self, input):
        ...
        hard_map = UserApi.getConfigItemValue("text-analysis-hard-actions")
        ...
        threshold = UserApi.getConfigItemValue("text-analysis-threshold")
        ...
        actions = UserApi.getConfigItemValue("text-analysis-actions")
