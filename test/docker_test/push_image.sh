...
process_image() {
    ...
    if [ -n "$2" ]; then
        if [ "$1" = "aarch64" ]; then
            export TARGET_IMAGE_TAG=$CI_REGISTRY/qorus/qorus/$IMAGE_NAME-$2-arm64
        else
            export TARGET_IMAGE_TAG=$CI_REGISTRY/qorus/qorus/$IMAGE_NAME-$2-amd64
        fi
    else
        if [ "$1" = "aarch64" ]; then
            export TARGET_IMAGE_TAG=$CI_REGISTRY/qorus/qorus/$IMAGE_NAME-arm64
        else
            export TARGET_IMAGE_TAG=$CI_REGISTRY/qorus/qorus/$IMAGE_NAME-amd64
        fi
    fi
    # tag as target
...
if [ "$MANUAL_IMAGE" != "1" ]; then
    ...
    export TARGET_IMAGE_TAG=$CI_REGISTRY/qorus/qorus/$IMAGE_NAME
