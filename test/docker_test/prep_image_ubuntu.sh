...
prep_image() {
    ...
    export FINAL_BRANCH=$(./test/docker_test/final_branch.sh)
    ...
    if [ -f "./test/dev-modules/TestUserConnectionProvider.qm" ]; then
        echo "qorus.connection-modules: ./test/dev-modules/TestUserConnectionProvider.qm" >>$OMQ_DIR/etc/options
    fi
